...
async function setupWebcam(videoTargetElement) {
    ...
    try {
        ...
        await new Promise(resolve => {
            videoTargetElement.onloadedmetadata = () => { videoTargetElement.play(); resolve(); };
        });
...
function connectToHost(hostID) {
    ...
    peer.on('error', err => {
        ...
        if (statusElement) statusElement.innerHTML = `<span style="color:#FF3333;">❌ Participant Error. Check Console.</span>`;
    });
}
